...
class CaficultorController extends Controller
{
    public function dashboard()
    {
        ...
        $kgVendidos = LoteCafe::where('caficultor_id', auth()->id())
            ->get()
            ->sum(fn($lote) => $lote->peso_kg - $lote->peso_disponible);
    ...
    public function guardarLote(Request $request)
    {
        ...
        try {
            ...
            return redirect()->route('caficultor.dashboard')
            ...
            return back()
                ->withInput()
                ->withErrors(['error' => 'Hubo un error al crear el lote: ' . $e->getMessage()]);
        }
    }
    ...
    public function actualizarLote(Request $request, $id)
    {
        ...
        $validated = $request->validate([
            'descripcion'  => 'nullable|string|max:1000',
            'precio_por_kg'=> 'required|numeric|min:1000',
            'estado'       => 'required|in:disponible,reservado,en_proceso,vendido',
        ]);
    ...
    public function eliminarLote($id)
    {
        ...
        if ($lote->codigo_qr && !str_starts_with($lote->codigo_qr, 'qr-google-')) {
            Storage::disk('public')->delete($lote->codigo_qr);
        }

        $lote->delete();

        // ✅ Redirige al dashboard (misma ruta que usa el original)
        return redirect()->route('caficultor.dashboard')
